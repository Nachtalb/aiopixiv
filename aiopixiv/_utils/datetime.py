# Source: https://github.com/python-telegram-bot/python-telegram-bot/blob/master/telegram/_utils/datetime.py
#
...
def _localize(datetime: dtm.datetime, tzinfo: dtm.tzinfo) -> dtm.datetime:
    ...
    return tzinfo.localize(datetime)  # type: ignore[attr-defined, no-any-return]
...
def to_float_timestamp(
    ...
    if isinstance(time_object, dtm.time):
        reference_dt = dtm.datetime.fromtimestamp(reference_timestamp, tz=time_object.tzinfo or tzinfo)
...
def to_timestamp(
    ...
    """
    return int(to_float_timestamp(dt_obj, reference_timestamp, tzinfo)) if dt_obj is not None else None
...
def from_timestamp(unixtime: Optional[int], tzinfo: dtm.tzinfo = UTC) -> Optional[dtm.datetime]:
    ...
    return dtm.datetime.utcfromtimestamp(unixtime)  # pyright: ignore[reportGeneralTypeIssues]
